...
def save_point_cloud(points_3d, filename, binary=True, with_label=False, verbose=True, intensity=False):
    ...
    with open(filename, 'w') as f:
      if intensity:
        f.write('ply\n'
                ...
                'end_header\n' % points_3d.shape[0])
        for row_idx in range(points_3d.shape[0]):
          X, Y, Z, R, G, B, I = points_3d[row_idx]
          f.write('%f %f %f %d %d %d %d 0\n' % (X, Y, Z, R, G, B, I))
      else:
          f.write('ply\n'
                'format ascii 1.0\n'
                'element vertex %d\n'
